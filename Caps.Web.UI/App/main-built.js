...
define('authentication',["Q","durandal/system","durandal/app","plugins/router","ko","infrastructure/antiForgeryToken","moment","infrastructure/utils"],function(e,t,n,o,r,i,a,s){function u(){var e=s.getFragment();return c().then(function(){return t.defer(function(t){sessionStorage.associatingExternalLogin?R(t,e):"undefined"!=typeof e.error?x(t,e):"undefined"!=typeof e.access_token?j(t,e):l().then(t.resolve)}).promise()})}function c(){return t.defer(function(e){$.ajax("~/api/account/authmetadata",{method:"get"}).then(function(t){O.lockoutPeriod=t.lockoutPeriod,O.minRequiredPasswordLength=t.minRequiredPasswordLength,e.resolve()}).fail(function(n){t.log("getMetadata failed: "+n.message),e.reject(n)})}).promise()}function l(){return t.defer(function(e){$.ajax("~/api/account/userinfo",{method:"get"}).then(function(t){A(new T(t.isAuthenticated,t.userName,t.roles,t)),t.IsAuthenticated&&n.trigger("caps:authentication:loggedOn",A()),e.resolve(A())}).fail(function(n){t.log("getCurrentUser failed: "+n.responseJSON.message),401===n.status?(A(new T(!1)),e.resolve(A())):e.reject(n)})}).promise()}function f(e,n){return t.defer(function(t){$.ajax("~/api/account/managementinfo",{method:"get",data:{returnUrl:e,generateState:n}}).then(function(e){t.resolve(e)}).fail(function(e){t.reject(e)})}).promise()}function d(e,n){return t.defer(function(t){$.ajax("~/api/account/externallogins",{method:"get",data:{returnUrl:e,generateState:n}}).then(t.resolve).fail(t.reject)}).promise()}function g(e){return t.defer(function(t){$.ajax("~/api/account/addexternallogin",{method:"post",data:{externalAccessToken:e}}).then(t.resolve).fail(t.reject)}).promise()}function m(e,n){return t.defer(function(t){$.ajax("~/api/account/removelogin",{method:"post",data:{loginProvider:e,providerKey:n}}).then(t.resolve).fail(t.reject)}).promise()}function h(e){return t.defer(function(t){$.ajax("~/api/account/setpassword",{method:"post",data:e}).then(t.resolve).fail(t.reject)}).promise()}function p(e){return t.defer(function(t){$.ajax("~/api/account/changepassword",{method:"post",data:e}).then(l).then(t.resolve).fail(t.reject)}).promise()}function v(o,r,a){return t.defer(function(s){function u(o){return o.userName&&o.access_token?(S(o.access_token,a),t.log("logon successful"),e.fcall(i.initToken).then(l).then(function(){n.trigger("caps:authentication:loggedOn",A()),s.resolve(o)}).fail(c),void 0):(c(new Error(y(o))),void 0)}function c(e){t.log("logon failed: "+e.message||e.error),s.reject(e)}$.ajax("~/Token?rememberMe="+a,{method:"post",data:{grant_type:"password",UserName:o,Password:r}}).done(u).fail(c)}).promise()}function w(){return t.defer(function(e){$.ajax("~/api/account/logout",{method:"post"}).then(b).then(i.initToken).then(function(){t.log("logoff successful"),n.trigger("caps:authentication:loggedOff"),A(new T),e.resolve()}).fail(e.reject)}).promise()}function R(e,t){var n,o;sessionStorage.removeItem("associatingExternalLogin"),"undefined"!=typeof t.error?(n=null,o=t.error):"undefined"!=typeof t.access_token?(n=t.access_token,o=null):(n=null,o=null),P(),l().then(function(t){t.userName?(window.location.hash="#profile",g(n).then(e.resolve(t))):e.reject()}).fail(e.reject)}function j(t,n){P(),S(n.access_token),e.fcall(i.initToken).then(l).then(function(e){E(),o.redirectFromLogonView(),t.resolve(e)})}function x(e,t){P(),e.reject(new Error(t.error))}function S(e,t){var n=t?localStorage:sessionStorage;n.accessToken=e}function k(){return sessionStorage.accessToken||localStorage.accessToken}function b(){localStorage.removeItem("accessToken"),sessionStorage.removeItem("accessToken")}function E(){var e=localStorage.logonSuccessRoute;e&&(localStorage.removeItem("logonSuccessRoute"),window.location.hash=e)}function P(){window.location.hash="","undefined"!=typeof history.pushState&&history.pushState("",document.title,location.pathname)}function y(e){var t="Die Anmeldung ist fehlgeschlagen. Versuche es in einigen Minuten nochmal. Melde das Problem, wenn es weiterhin besteht.";if(e){var n=e.Error;if(n){if("ERROR_LOCKED"===n){var o="Dein Konto wurde aufgrund zu vieler ungültiger Anmelde-Versuche gesperrt. Die Sperrung wird nach {0} Minuten automatisch aufgehoben.";return o.replace(/\{0\}/gi,O.lockoutPeriod)}if("ERROR_NOTAPPROVED"===n)return"Dein Konto wurde noch nicht bestätigt.";if("ERROR_USER_OR_PASSWORD_INVALID"===n)return"Der Benutzername oder das Passwort sind ungültig.";if("Bad request"===n)return t}}return e.Error||t}function N(){return t.defer(function(e){I()&&!A().isExpired()?e.resolve(A()):A().isExpired()?l().then(e.resolve).fail(e.reject):e.resolve(A())}).promise()}function T(e,t,n,o,i){var a=this;o=o||{},a.expirationTicket=new D(i),a.isAuthenticated=r.observable(e||!1),a.userName=r.observable(t||""),a.roles=r.observable(n||[]),a.creationDate=r.observable(o.creationDate||new Date),a.lastPasswordChangedDate=r.observable(o.lastPasswordChangedDate),a.firstName=r.observable(o.firstName||""),a.lastName=r.observable(o.lastName||""),a.displayName=r.computed(function(){return a.firstName().length>0?a.firstName():a.lastName().length>0?a.lastName():a.userName()}),a.fullName=r.computed(function(){return"{0} {1}".replace(/\{0\}/,a.firstName()).replace(/\{1\}/,a.lastName()).trim()}),a.hasEverChangedPassword=r.computed(function(){return a.lastPasswordChangedDate()>a.creationDate()}),a.isInRole=function(e){for(var t=0;t<this.roles().length;t++)if(a.roles()[t]===e)return!0;return!1},a.isInAnyRole=function(e){if(!e||0===e.length)return!0;for(var t=0;t<e.length;t++)if(a.isInRole(e[t]))return!0;return!1}}function D(e){this.created=new Date,this.expiration=e||!1}var A=r.observable(new T),I=r.computed(function(){return A().isAuthenticated()}),O={lockoutPeriod:15,minRequiredPasswordLength:6},_="viewmodels/login",L="login",q="";return $(document).ajaxSend(function(e,t){var n=k();n&&t.setRequestHeader("Authorization","Bearer "+n)}),$(document).ajaxError(function(e,t,n,r){(401===t.status||"POST"===n.type&&403===t.status)&&o.navigate(o.logon(o.activeInstruction())),console.log("AJAX ERROR: "+r)}),o.logon=function(e){if(e&&e.config.moduleId===_)throw new Error("The logon-Function may not be called with the logon-route.");return o.logonSuccessRoute=e,L},o.redirectFromLogonView=function(){var e=q;if(o.logonSuccessRoute){var t=o.logonSuccessRoute;delete o.logonSuccessRoute,e="#"+t.fragment}o.navigate(e,{trigger:!0,replace:!0})},o.guardRoute=function(e,n){return t.defer(function(e){N().then(function(){I()||n.config.moduleId===_?(n.config.roles&&!A().isInAnyRole(n.config.roles)&&e.resolve("forbidden"),e.resolve(!0)):e.resolve(o.logon(n))}).fail(e.reject)}).promise()},T.prototype.isExpired=function(){return this.expirationTicket.isExpired()},D.prototype.isExpired=function(){return this.expiration?a(this.created).add("seconds",this.expiration)<new Date:!1},{initialize:u,metadata:O,user:A,logon:v,logoff:w,changePassword:p,setPassword:h,isAuthenticated:I,UserModel:T,getAccountManagementInfo:f,getExternalLoginProviders:d,removeLogin:m}});
...
define('modules/sitemap/viewmodels/index',["../module","ko","plugins/router","durandal/system","durandal/app","localization","./siteMapViewModel","./publicationViewModel","infrastructure/publicationService","authentication","../datacontext","durandal/composition","moment","infrastructure/keyboardHandler","infrastructure/scrollState","./addPageDialog"],function(e,t,n,r,i,a,o,s,l,u,c,d,f,p,h,m){function v(e){x(e)&&c.fetchSiteMapNode(e.Id()).then(function(){_()&&_().refreshTree(),y()})}function g(e){A(null),e&&e.fetchTree().then(function(){if(A()){var t=e.tree().findNodeByKey(A().Id());if(t)return t.selectNode(),void 0}e.selectRootNode()})}function b(e){L(null),y(),w(e)}function y(){V(null),A()&&A().ContentId()&&c.fetchPublication(A().ContentId()).then(function(){var e=new s(A());V(e)}).fail(N)}function w(e){O(null),e&&O(new P(e))}function k(e){var n=t.utils.arrayFirst(j.siteMapVersions(),function(t){return t.entity()===e});_(n)}function C(e){var t=new o(e);return t.selectedNodeChanged=function(e){e&&A(e.entity())},t}function x(e){return _()&&_().containsNode(e)}function N(e){r.log(e.message),alert(e.message)}function D(e){var t=_();if(t&&e){var n=e.nextNode()||e.previousNode()||e.ParentNode();c.deleteSiteMapNode(e).then(function(){t.refreshTree(),n&&t.selectNodeByKey(n.Id()),i.trigger("caps:sitemapnode:deleted",e)}).fail(N)}}function I(){c.fetchFirstWebsite().then(function(e){R(e.results[0]),k(R().latestSiteMap())}).fail(N)}function F(e){c.fetchSiteMapNodeByContentId(e).then(function(e){e.results.length?M(e.results[0]):I()}).fail(N)}function S(e){c.fetchSiteMapNode(e).then(function(e){e.results.length?M(e.results[0]):I()}),fail(N)}function M(e){var t=e.SiteMap();R(t.Website()),k(t),A(e)}function T(e){var n=this;n.entity=e,n.isSelected=t.computed(function(){return L()===n}),n.selectTeaser=function(){L(n),w(n.entity)}}function P(n){var r=this;r.entity=n,r.supportedTranslations=a.website.supportedTranslations(),r.edit=function(){n&&e.router.navigate("#sitemap/edit/"+n.Id())},r.editTranslation=function(t){n&&e.router.navigate("#sitemap/translate/"+n.Id()+"/"+t.culture)},r.createTeaser=function(){if(n){var e=n.SiteMap().Id(),t=n.SiteMap().rootNodes()[0].Id();l.createTeaser(e,t,n.Id())}},r.canCreateTeaser=function(){return n?"ROOT"===n.NodeType()||n.hasTeasers()?!1:n.isTeaser()?!1:!0:!1},r.deleteTeaser=function(){var e="Aufmacher löschen",t="Abbrechen";i.showMessage('Soll der Aufmacher "'+r.entity.localeTitle("de")+'" wirklich gelöscht werden?',"Aufmacher löschen",[e,t]).then(function(t){t===e&&D(r.entity)})},r.hasOptions=t.computed(function(){return r.entity.isTeaser()}),r.hasContent=t.computed(function(){return n&&n.Content()}),r.contentSummary=t.computed(function(){if(!n.ContentId())return"Kein Inhalt festgelegt";var e=n.Content();return e?e.EntityType()+" #"+e.EntityKey()+" (v."+e.ContentVersion()+")":""}),r.authorSummary=t.computed(function(){if(!n.ContentId()||!n.Content())return"";var e=n.Content();return e.AuthorName()+" "+f(e.ContentDate()).fromNow()});var o;r.selectContent=function(){function t(e){l.setNodeContent(r.entity.Id(),e).fail(function(e){alert(e.message)})}o=!1,i.selectContent({module:e}).then(function(e){e.dialogResult&&t(e.selectedContent)}).finally(function(){o=!0})},r.editContent=function(){n.ContentId()&&n.Content()&&i.editContent(n.Content().EntityType(),n.Content().EntityKey())},r.deletePublication=function(){function e(){n.ContentId()&&n.Content()&&c.deletePublication(n)}var t="Entfernen",r="Abbrechen";i.showMessage("Soll der Inhalt dieser Seite wirklich entfernt werden?","Inhalt entfernen?",[t,r]).then(function(n){n===t&&e()})}}var E=$(window),R=t.observable(),_=t.observable(),A=t.observable(),V=t.observable(),L=t.observable(),O=t.observable(),B=!1,U=new p(e),z=new h(e);_.subscribe(g),A.subscribe(b),i.on("caps:sitemapnode:saved",function(e){x(e)&&c.fetchSiteMapNode(e.Id())}),i.on("caps:publication:created",v),i.on("caps:publication:refreshed",v),e.on("module:compositionComplete",function(e,t){t===j&&(E.trigger("forceViewportHeight:refresh"),z.activate())});var j={website:R,siteMapVersions:t.computed(function(){var e=R()?R().sortedSiteMapVersions():[];return t.utils.arrayMap(e,C)}),selectedSiteMap:_,selectedNode:A,selectedPublication:V,properties:O,listScrollState:z,teasers:t.computed(function(){if(!A())return[];var e=t.utils.arrayFilter(A().childNodes(),function(e){return e.isTeaser()});return t.utils.arrayMap(e,function(e){return new T(e)})}),activate:function(e){B||(B=!0,e&&(e.p||e.n)||I()),e&&(e.p&&F(e.p),e.n&&S(e.n)),U.activate()},deactivate:function(){U.deactivate(),z.deactivate()},createSiteMapVersion:function(){c.createNewSiteMapVersion(R()).then(k).fail(N)},deleteSiteMapVersion:function(){var e="Sitemap-Version verwerfen",t="Abbrechen",n=_().entity();n&&i.showMessage("Soll diese Version wirklich verworfen werden?","Sitemap-Version verwerfen?",[e,t]).then(function(t){if(t===e){var r=n.nextVersion()||n.previousVersion();k(r),c.deleteSiteMapVersion(n).fail(N)}})},publishSiteMapVersion:function(){c.publishSiteMap(_().entity(),u.user().userName())},editWebsite:function(){n.navigate("#website")},moveSelectedNodeUp:function(){A()&&(A().moveUp(),c.saveChanges().then(function(){_()&&_().refreshTree()}))},moveSelectedNodeDown:function(){A()&&(A().moveDown(),c.saveChanges().then(function(){_()&&_().refreshTree()}))},canAddPage:function(){return A()?!0:!1},addPage:function(){function e(e){c.createSiteMapNode(_().entity(),A(),e,e).then(function(e){_().refreshTree(),_().selectNodeByKey(e.Id())}).fail(N)}_()&&A()&&m.show().then(function(t){t&&e(t.title)})},canEditContent:function(){return A()&&O()&&O().hasContent()?!0:!1},editContent:function(){O()&&O().editContent()},canSelectContent:function(){return A()?!0:!1},selectContent:function(){O()&&O().selectContent()},canMoveUp:function(){return A()&&"ROOT"!==A().NodeType()?!0:!1},canMoveDown:function(){return A()&&"ROOT"!==A().NodeType()?!0:!1},canMoveToNode:function(){return A()&&"ROOT"!==A().NodeType()?!0:!1},moveToNode:function(){function t(e){return!e.isTeaser()&&e.Id()!==A().Id()}if(_()){var n=_(),r=A();i.selectSiteMapNode({module:e,nodeFilter:t,siteMapId:n.entity().Id(),canSelectSiteMap:!1}).then(function(e){e.dialogResult&&e.selectedNode&&e.selectedNode.Id()!==r.ParentNodeId()&&(r.reparent(e.selectedNode),c.saveChanges().then(function(){n.refreshTree(),n.tree().selectedNode().ensureVisible()}))})}},canDeleteNode:function(){return A()&&"ROOT"!==A().NodeType()?!0:!1},deleteNode:function(){var e="Seite löschen",t="Abbrechen";i.showMessage('Soll die Seite "'+A().localeTitle("de")+'" wirklich gelöscht werden?',"Seite löschen",[e,t]).then(function(t){t===e&&D(A())})}};return U.keydown=function(e){var t=document.activeElement;(!t||"INPUT"!==t.tagName&&"TEXTAREA"!==t.tagName)&&_()&&_().tree()&&_().tree().handleKeyDown(e)},j});
