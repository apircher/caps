...
define('modules/sitemap/datacontext',["entityManagerProvider","breeze","durandal/system"],function(e,t,n){function r(e){var t=p.createEntity("DbSiteMap",{WebsiteId:e.Id(),Version:1}),n=p.createEntity("DbSiteMapNode",{NodeType:"ROOT",Name:"HOME"}),r=n.getOrCreateResource("de",p);return n.SiteMapId(t.Id()),r.Title("Startseite"),p.addEntity(n),t}function i(e){return n.defer(function(t){e||t.reject(new Error("No website provided."));var n,i=e.latestSiteMap(),a=i?i.Version()+1:1;n=i?i.createNewVersion(a,p):r(p),p.saveChanges().then(function(){t.resolve(n)}).fail(t.reject)}).promise()}function a(e){return n.defer(function(t){e.setDeleted(),p.saveChanges().then(t.resolve).fail(t.reject)}).promise()}function o(e,t){return e.PublishedFrom(new Date),e.PublishedBy(t),p.saveChanges()}function s(e,t){return n.defer(function(n){e||n.reject(new Error("No siteMap provided")),t||n.reject(new Error("No parentNode provided"));var r=p.createEntity("DbSiteMapNode",{NodeType:"PAGE"});p.addEntity(r),r.ParentNodeId(t.Id()),r.SiteMapId(e.Id());var i=r.getOrCreateResource("de",p);i.Title("Seite "+(new Date).toLocaleTimeString()),p.saveChanges().then(function(){n.resolve(r)}).fail(n.reject)}).promise()}function l(e){return e.setDeleted(),p.saveChanges()}function u(e){return n.defer(function(t){var n=(new m).from("Publications").where("Id","==",e).expand("Translations, ContentParts.Resources, Files.Resources.FileVersion.File");p.executeQuery(n).then(function(e){t.resolve(e.results[0])}).fail(t.reject)}).promise()}function c(){var e=(new m).from("Websites").expand("SiteMapVersions").take(1);return p.executeQuery(e)}function d(e){var t=(new m).from("SiteMapNodes").where("Id","==",e).expand("Resources");return p.executeQuery(t)}function f(e){var t=(new m).from("SiteMapNodes").where("ContentId","==",e).expand("SiteMap.Website.SiteMapVersions");return p.executeQuery(t)}function h(e){var t=new m("SiteMapNodes").where("SiteMapId","==",e).expand("Resources");return p.executeQuery(t)}var p=e.createManager(),m=t.EntityQuery;return{createInitialSiteMap:r,createNewSiteMapVersion:i,deleteSiteMapVersion:a,publishSiteMap:o,createSiteMapNode:s,deleteSiteMapNode:l,fetchPublication:u,fetchFirstWebsite:c,fetchSiteMapNode:d,fetchSiteMapNodes:h,fetchSiteMapNodeByContentId:f,saveChanges:function(){return p.saveChanges()}}});
