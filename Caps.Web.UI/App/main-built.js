...
define('modules/draft/viewmodels/editor/editor',["durandal/app","durandal/system","../../module","../../datacontext","../../entities","entityManagerProvider","breeze","ko","Q","./navigation","./draftTemplate","./draftProperties","./draftFiles","./contentPartEditor","./templateEditor","./draftNotes","./editorModel","authentication"],function(e,t,n,r,i,a,o,s,l,c,u,d,f,h,p,m,v,g){function b(){function l(){y.trigger("forceViewportHeight:refresh")}function v(e){var t=e.Id(),n=s.utils.arrayFilter(T.entity().Files(),function(e){var n=s.utils.arrayFirst(e.Resources(),function(e){return e.FileVersion()&&e.FileVersion().FileId()===t});return null!=n});if(n.length){var r=o.EntityQuery.from("Files").where("Id","==",t).expand("Versions.File");return E.executeQuery(r)}}function b(e,t,n){n=n||"de",r.getTemplate(e).then(function(r){var i=r,a=E.createEntity("Draft",{TemplateName:e,Version:1,OriginalLanguage:n,Status:"NEW"});a.Template(JSON.stringify(i)),a.Created().At(new Date),a.Created().By(g.user().userName()),a.Modified().At(new Date),a.Modified().By(g.user().userName()),t&&t.name&&a.Name(t.name),T.entity(a),i.forEachCell(function(e,n,r){var i=T.getOrCreateContentPart(n);if(i.Ranking(r),n.content){var a=/<%\s*([A-Za-z0-9_\.]+)\s*%>/gi,o=n.content.replace(a,function(e,n){return t&&t[n]?t[n]:w(n)||"Nicht gefunden"});i.getResource("de").Content(o)}})})}function w(e){if(!T.entity()||!T.entity().template())return null;var t=T.entity().template();if(!t.parameters)return null;var n=s.utils.arrayFirst(t.parameters,function(t){return t.name.toLowerCase()===e.toLowerCase()});return n?n.value:null}function k(e){var t=o.EntityQuery.from("Drafts").where("Id","==",e).expand("ContentParts.Resources, Files.Resources.FileVersion.File");return E.executeQuery(t).then(function(e){T.entity(e.results[0])})}function x(){T.entity().setDeleted(),E.saveChanges().then(function(){e.trigger("caps:draft:deleted",T.entity()),T.navigateBack()})}function C(){D=D||new c(T);var e=L.activeInstruction().queryParams;if(e&&e.t)if("DraftProperties"===e.t)T.showProperties();else if("DraftFiles"===e.t)T.showFiles();else{var t=T.entity().findContentPart(e.t);t&&T.showContentPartEditor(t)}T.currentContent()||(F=F||new u(T),T.currentContent(F)),T.currentNavigation(D)}function N(e){var t=s.utils.arrayFirst(_,function(t){return t.contentPart===e});return t||(t=new h(T,e),_.push(t)),t}var D,F,S,I,M,P,T=this,E=a.createManager(),_=[],L=n.router,R=s.observable();T.currentContent=s.observable(),T.currentNavigation=s.observable(),T.entity=s.observable(),T.template=s.computed(function(){return T.entity()?T.entity().template():void 0}),T.isNewDraft=s.observable(!1),T.draftStates=i.supportedDraftStates,T.lastContentPart=s.observable(),E.hasChangesChanged.subscribe(function(e){n.routeConfig.hasUnsavedChanges(e.hasChanges)}),E.validationErrorsChanged.subscribe(function(e){var t=e.entity;R(t.hasValidationErrors.call(t))}),T.activate=function(r,i){return n.on("module:compositionComplete",l),e.on("caps:contentfile:uploadDone",v),t.defer(function(e){r&&/^[0-9]+$/.test(r)?k(r).then(function(){C(),e.resolve()}):(T.isNewDraft(!0),b(r,i),C(),e.resolve())}).promise()},T.canDeactivate=function(){return E.hasChanges()?t.defer(function(t){var n="Speichern",r="Verwerfen",i="Abbrechen";e.showMessage("Sollen die Änderungen gespeichert werden?","Änderungen speichern?",[n,r,i]).then(function(e){if(e===i)t.resolve(!1);else if(e===n){var r=T.saveChanges();r&&r.then?r.then(function(){t.resolve(!0)}):t.resolve(!1)}else t.resolve(!0)})}).promise():!0},T.deactivate=function(){n.routeConfig.hasUnsavedChanges(!1),n.off("module:compositionComplete",l),e.off("caps:contentfile:uploadDone",v),S&&S.deactivate()},T.shouldActivate=function(e,t,n){return t[0]!==n[0]},T.showEditorMain=function(){F=F||new u(T),T.currentContent(F),T.lastContentPart(null)},T.showLayoutArea=function(){T.lastContentPart()?T.showContentPartEditor(T.lastContentPart()):T.showEditorMain()},T.showFiles=function(){S=S||new f(T),T.currentContent(S)},T.showProperties=function(){I=I||new d(T),T.currentContent(I)},T.showContentPartEditor=function(e){var t=N(e);t&&(t.showPreview(!1),T.currentContent(t),T.lastContentPart(e))},T.showTemplateEditor=function(){M=M||new p(T),T.currentContent(M)},T.showDraftNotes=function(){P=P||new m(T),T.currentContent(P)},T.navigateBack=function(){T.showDraftsIndex()},T.showDraftsIndex=function(){n.router.navigate(n.routeConfig.hash)},T.saveChangesAndNavigateBack=function(){T.saveChanges().then(T.navigateBack)},T.saveChanges=function(){if(R())return e.showMessage("Der Inhalt kann noch nicht gespeichert werden. Prüfe die markierten Felder und ergänze oder korrigiere die Eingaben.","Noch nicht komplett",["Ok"]),!1;T.entity().Modified().At(new Date),T.entity().Modified().By(g.user().userName());var t=E.getEntities("DraftFile",o.EntityState.Deleted);return E.saveChanges().then(function(){e.trigger("caps:draft:saved",{entity:T.entity(),isNewDraft:T.isNewDraft(),deletedFiles:t}),n.routeConfig.hasUnsavedChanges(!1)})},T.deleteDraft=function(){var t="Entwurf löschen",n="Abbrechen";e.showMessage("Soll der Entwurf wirklich gelöscht werden?","Entwurf löschen",[t,n]).then(function(e){if(e===t){if(T.isNewDraft())return E.rejectChanges(),T.navigateBack(),void 0;x()}})},T.getOrCreateContentPart=function(e){var t=T.entity().findContentPart(e.name);if(!t){t=E.createEntity("DraftContentPart",{DraftId:T.entity().Id(),Name:e.name,ContentType:e.contentType||"markdown"}),E.addEntity(t);var n=E.createEntity("DraftContentPartResource",{DraftContentPartId:T.entity().Id(),Language:"de",Content:""});E.addEntity(n),t.Resources.push(n),T.entity().ContentParts.push(t)}return t},T.createDraftFile=function(e,t){var n=o.EntityQuery.from("Files").where("Id","==",e.Id()).expand("Versions.File");return E.executeQuery(n).then(function(n){var r=n.results[0],i=E.createEntity("DraftFile",{Name:e.FileName()});e.isImage()&&i.Determination("Picture"),t&&t.length&&i.Group(t),E.addEntity(i);var a=E.createEntity("DraftFileResource",{Language:"de",DbFileVersionId:r.latestVersion().Id()});return E.addEntity(a),i.Resources.push(a),i.DraftId(T.entity().Id()),T.entity().Files.push(i),i})}}var y=$(window);return b});
