...
define('modules/draft/datacontext',["durandal/system","entityManagerProvider","ko","infrastructure/userQueryParser","./entities","./stockTemplates"],function(e,t,n,r,i,a){function o(){var e=p.from("Drafts");return f.executeQuery(e)}function s(e,t){var n=p.from("Drafts");if(e&&e.length){var r=h.getBreezePredicate(e);r&&(n=n.where(r))}return n=n.orderBy(t||"Created.At desc"),f.executeQuery(n)}function l(e){var t=(new p).from("Drafts").where("Id","==",e).expand("Translations, ContentParts.Resources, Files.Resources.FileVersion.File");return f.executeQuery(t)}function c(t){return e.defer(function(e){var r=n.utils.arrayFirst(a.all(),function(e){return e.name.toLowerCase()===t.toLowerCase()});r&&e.resolve(r);var o=(new p).from("DraftTemplates").where("Name","==",t);f.executeQuery(o).then(function(t){t.results&&t.results.length||e.resolve(null);try{r=JSON.parse(t.results[0].TemplateContent()),i.initializeTemplate(r)}catch(n){e.reject(n)}e.resolve(r)}).fail(e.reject)}).promise()}function d(t,n){return n=n||f,e.defer(function(e){u(t,n);var r=new breeze.Predicate("Content.EntityType","==","Draft").and("Content.EntityKey","==",t),i=(new p).from("SiteMapNodes").where(r).expand("Content, SiteMap, SiteMap.SiteMapNodes, SiteMap.SiteMapNodes.Resources");n.executeQuery(i).then(function(t){e.resolve(t.results)}).fail(e.reject)}).promise()}function u(e,t){t=t||f;var n=new breeze.Predicate("Content.EntityType","==","Draft").and("Content.EntityKey","==",e),r=(new p).from("SiteMapNodes").where(n),i=t.executeQueryLocally(r);i.forEach(function(e){t.detachEntity(e)})}var f=t.createManager(),p=breeze.EntityQuery,h=new r;return h.translateColumnName=function(){return"Name"},{getDrafts:o,getDraft:l,getTemplate:c,fetchPublications:d,searchDrafts:s,isValidUserQuery:function(e){return h.validate(e)}}});
...
define('modules/draft/viewmodels/editor/editor',["durandal/app","durandal/system","../../module","../../datacontext","../../entities","entityManagerProvider","breeze","ko","Q","./navigation","./draftTemplate","./draftProperties","./draftFiles","./contentPartEditor","./templateEditor","./draftNotes","./editorModel","authentication"],function(e,t,n,r,i,a,o,s,l,u,c,d,f,p,h,m,v,g){function b(){function l(){y.trigger("forceViewportHeight:refresh")}function v(e){var t=e.Id(),n=s.utils.arrayFilter(_.entity().Files(),function(e){var n=s.utils.arrayFirst(e.Resources(),function(e){return e.FileVersion()&&e.FileVersion().FileId()===t});return null!=n});if(n.length){var r=o.EntityQuery.from("Files").where("Id","==",t).expand("Versions.File");return R.executeQuery(r)}}function b(e,t,n){n=n||"de",r.getTemplate(e).then(function(r){var i=r,a=R.createEntity("Draft",{TemplateName:e,Version:1,OriginalLanguage:n,Status:"NEW"});a.Template(JSON.stringify(i)),a.Created().At(new Date),a.Created().By(g.user().userName()),a.Modified().At(new Date),a.Modified().By(g.user().userName()),t&&t.name&&a.Name(t.name),_.entity(a),i.forEachCell(function(e,n,r){var i=_.getOrCreateContentPart(n);if(i.Ranking(r),n.content){var a=/<%\s*([A-Za-z0-9_\.]+)\s*%>/gi,o=n.content.replace(a,function(e,n){return t&&t[n]?t[n]:w(n)||"Nicht gefunden"});i.getResource("de").Content(o)}})})}function w(e){if(!_.entity()||!_.entity().template())return null;var t=_.entity().template();if(!t.parameters)return null;var n=s.utils.arrayFirst(t.parameters,function(t){return t.name.toLowerCase()===e.toLowerCase()});return n?n.value:null}function k(e){var t=o.EntityQuery.from("Drafts").where("Id","==",e).expand("ContentParts.Resources, Files.Resources.FileVersion.File");return R.executeQuery(t).then(function(e){_.entity(e.results[0])})}function x(){_.entity().setDeleted(),R.saveChanges().then(function(){e.trigger("caps:draft:deleted",_.entity()),_.navigateBack()})}function C(){I=I||new u(_);var e=A.activeInstruction().queryParams;if(e&&e.t)if("DraftProperties"===e.t)_.showProperties();else if("DraftFiles"===e.t)_.showFiles();else{var t=_.entity().findContentPart(e.t);t&&_.showContentPartEditor(t)}_.currentContent()||(S=S||new c(_),_.currentContent(S)),_.currentNavigation(I)}function N(){var e=_.entity();e&&e.entityAspect.propertyChanged.subscribe(D)}function D(){n.routeConfig.hasUnsavedChanges(R.hasChanges())}function F(e){var t=s.utils.arrayFirst(L,function(t){return t.contentPart===e});return t||(t=new p(_,e),L.push(t)),t}var I,S,M,T,E,P,_=this,R=a.createManager(),L=[],A=n.router;_.currentContent=s.observable(),_.currentNavigation=s.observable(),_.entity=s.observable(),_.entity.subscribe(N),_.template=s.computed(function(){return _.entity()?_.entity().template():void 0}),_.isNewDraft=s.observable(!1),_.draftStates=i.supportedDraftStates,_.lastContentPart=s.observable(),_.activate=function(r,i){return n.on("module:compositionComplete",l),e.on("caps:contentfile:uploadDone",v),t.defer(function(e){r&&/^[0-9]+$/.test(r)?k(r).then(function(){C(),e.resolve()}):(_.isNewDraft(!0),b(r,i),C(),e.resolve())}).promise()},_.deactivate=function(){n.off("module:compositionComplete",l),e.off("caps:contentfile:uploadDone",v),M&&M.deactivate()},_.shouldActivate=function(e,t,n){return t[0]!==n[0]},_.showEditorMain=function(){S=S||new c(_),_.currentContent(S),_.lastContentPart(null)},_.showLayoutArea=function(){_.lastContentPart()?_.showContentPartEditor(_.lastContentPart()):_.showEditorMain()},_.showFiles=function(){M=M||new f(_),_.currentContent(M)},_.showProperties=function(){T=T||new d(_),_.currentContent(T)},_.showContentPartEditor=function(e){var t=F(e);t&&(t.showPreview(!1),_.currentContent(t),_.lastContentPart(e))},_.showTemplateEditor=function(){E=E||new h(_),_.currentContent(E)},_.showDraftNotes=function(){P=P||new m(_),_.currentContent(P)},_.navigateBack=function(){_.showDraftsIndex()},_.showDraftsIndex=function(){n.routeConfig.hasUnsavedChanges(!1),n.router.navigate(n.routeConfig.hash)},_.saveChanges=function(){_.entity().Modified().At(new Date),_.entity().Modified().By(g.user().userName());var t=R.getEntities("DraftFile",o.EntityState.Deleted);R.saveChanges().then(function(){e.trigger("caps:draft:saved",{entity:_.entity(),isNewDraft:_.isNewDraft(),deletedFiles:t}),_.showDraftsIndex()})},_.deleteDraft=function(){var t="Entwurf löschen",n="Abbrechen";e.showMessage("Soll der Entwurf wirklich gelöscht werden?","Entwurf löschen",[t,n]).then(function(e){e===t&&x()})},_.getOrCreateContentPart=function(e){var t=_.entity().findContentPart(e.name);if(!t){t=R.createEntity("DraftContentPart",{DraftId:_.entity().Id(),Name:e.name,ContentType:e.contentType||"markdown"}),R.addEntity(t);var n=R.createEntity("DraftContentPartResource",{DraftContentPartId:_.entity().Id(),Language:"de",Content:""});R.addEntity(n),t.Resources.push(n),_.entity().ContentParts.push(t)}return t},_.createDraftFile=function(e,t){var n=o.EntityQuery.from("Files").where("Id","==",e.Id()).expand("Versions.File");return R.executeQuery(n).then(function(n){var r=n.results[0],i=R.createEntity("DraftFile",{Name:e.FileName()});e.isImage()&&i.Determination("Picture"),t&&t.length&&i.Group(t),R.addEntity(i);var a=R.createEntity("DraftFileResource",{Language:"de",DbFileVersionId:r.latestVersion().Id()});return R.addEntity(a),i.Resources.push(a),i.DraftId(_.entity().Id()),_.entity().Files.push(i),i})}}var y=$(window);return b});
