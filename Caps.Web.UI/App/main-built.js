...
define('modules/contentfile/viewmodels/uploadManager',["ko","durandal/system","durandal/app","./../datacontext","./fileUploadDialog"],function(e,t,n,r,i){function a(){function t(t,a){var o=e.utils.arrayMap(t,function(e){return e.name});r.getFileInfo(o).then(function(t){var r=e.utils.arrayFilter(t,function(e){return e.count>0});if(r.length>0){var o=new i(t);n.showDialog(o).then(function(e){e&&a(e.storageOption,r)})}else a()})}var a=this,o=new s;a.isUploading=e.observable(!1),a.progress=e.observable(0),a.currentUploads=e.observableArray(),a.addFiles=function(e,n){var r=o.findOrAddBatch(n);r&&(r.addFiles(n),r.files.length===n.originalFiles.length&&("replace"===r.storageOption?r.submit():t(r.originalFiles,function(e,t){e&&(r.storageOption=e),t&&(r.filesExistingOnServer=t),r.submit()})))},a.filesDropped=function(e){var t;return t=e&&e.delegatedEvent?e.delegatedEvent:e,t&&t.dataTransfer&&"copy"===t.dataTransfer.dropEffect?!1:void 0},a.uploadDone=function(t,r){e.utils.arrayForEach(r.result,function(t){var i=e.utils.arrayFirst(r.files,function(e){return e.name===t.fileName});a.currentUploads.remove(i),n.trigger("uploadManager:uploadDone",i,t)}),a.isUploading(!1)},a.uploadFailed=function(t,n){e.utils.arrayForEach(n.files,function(e){e.listItem.isUploading(!1)}),a.isUploading(!1)},a.uploadProgress=function(e,t){var n=parseInt(100*(t.loaded/t.total),10);a.progress(n)}}function o(t){function r(t){return!!e.utils.arrayFirst(a.filesExistingOnServer,function(e){return e.fileName===t})}function i(e){var t={};if(e.fileInput){var n=e.fileInput.data("replace-id");n&&(t.versionId=n)}return t}var a=this;a.originalFiles=t,a.files=[],a.storageOption="add",a.filesExistingOnServer=[],a.addFiles=function(e){a.files.push(e)},a.submit=function(){var t=0;e.utils.arrayForEach(a.files,function(e){var o=e.files[0],s=a.storageOption;a.filesExistingOnServer&&a.filesExistingOnServer.length>0&&!r(o.name)&&(s=void 0),n.trigger("uploadManager:uploadStarted",o,t++,s),n.uploadManager.currentUploads.push(o),e.formData=i(e),e.formData.storageAction=s,e.submit()})}}function s(){var t=this,n=[];t.findOrAddBatch=function(e){var r=t.findBatch(e.originalFiles);if(!r){if(r=new o(e.originalFiles),e.fileInput){var i=e.fileInput.data("storage-option");i&&(r.storageOption=i)}n.push(r)}return r},t.findBatch=function(t){return e.utils.arrayFirst(n,function(e){return e.originalFiles===t})},t.removeBatch=function(e){var r=t.findBatch(e);if(r){var i=n.indexOf(r);n.splice(i,1)}}}return a});
...
define('modules/contentfile/viewmodels/index',["ko","durandal/system","durandal/app","../module","../datacontext","infrastructure/virtualListModel","toastr","Q","doubleTap","./fileListItem","./uploadManager","./fileSearchControl","infrastructure/serverUtil"],function(e,t,n,i,a,o,s,l,c,d,u,f,h){function p(e){var n=N,r=n.hasFilterOptions()?n.currentFilter:"",i=n.sortOptions.getOrderBy();return console.log("loadPage called. pageNumber="+e+", Filter="+r),t.defer(function(t){y(!0),a.searchFiles(n.searchWords(),e,b.itemsPerPage(),i,r).then(function(n){b.addPage(n,e),t.resolve()}).fail(t.reject).done(function(){y(!1)})}).promise()}function v(e){function t(){b.selectedItem()===e&&b.resetSelection(),b.removeItem(e)}function r(){n.showMessage("Die Datei konnte nicht gelöscht werden.","Nicht erfolgreich")}return a.deleteFile(e.data()).then(t).fail(r)}var m,g=!1,b=new o.VirtualList(35,null,d),y=e.observable(!1),w=n.uploadManager,k=e.observable(!1),x=e.observable(0),N=new f;return i.router.on("router:navigation:attached",function(e){e==m&&k(!0)}),n.on("caps:tags:added",function(e){N.addTagFilter(e)}),n.on("caps:tag:deleted",function(e){N.removeTagFilter(e)}),n.on("caps:file:deleted",function(e){var t=b.findItem(function(t){return t.data().Id()==e.Id()});t&&(b.selectedItem()===t&&b.resetSelection(),b.removeItem(t))}),n.on("uploadManager:uploadStarted",function(e,t,n){var r="replace"===n;if(!r){var i=b.insertItem(void 0,t);i.isUploading(!0),i.tempData=e}}),n.on("uploadManager:uploadDone",function(e,t){var n=b.findItem(function(n){return n.tempData===e||n.data()&&n.data().Id()===t.id});a.fetchFile(t.id).then(function(){n&&(n.data(a.localGetFile(t.id)),n.isUploading(!1))}).fail(function(){s.error("Die Datei "+r.FileName+" konnte nicht geladen werden.")})}),N.refreshResults=function(){m.refresh()},m={list:b,isLoading:y,uploadManager:w,selectedFile:b.selectedItem,selectedFiles:b.selectedItems,scrollTop:x,isInteractive:k,searchControl:N,server:h,activate:function(){g||(g=!0,p(1))},deactivate:function(){k(!1)},deleteFile:function(e){var t="Datei löschen",r="Abbrechen";n.showMessage("Soll die Datei "+e.data().FileName()+" wirklich gelöscht werden?","Datei löschen",[t,r]).then(function(n){n===t&&v(e)})},deleteSelection:function(){var e=this.selectedFiles();if(0!==e.length){if(1==e.length)return this.deleteFile(e[0]);var t="Auswahl löschen";n.showMessage("Sollen die "+e.length+" ausgewählten Dateien wirklich endgültig gelöscht werden?","Auswahl löschen",[t,"Abbrechen"]).then(function(n){if(n===t){b.suspendEvents=!0;var r=l.when({});e.forEach(function(e){r=r.then(function(){return v(e)})}),l.when(r).then(function(){b.suspendEvents=!1,b.raiseItemsChanged()})}})}},refresh:function(){x(0),b.resetSelection(),b.removeAll(),p(1)},resetSelectedItem:function(){b.resetSelection()},showSelectedFile:function(){this.showDetail(b.selectedItem())},showDetail:function(e){e&&(b.selectItem(e),i.router.navigate("#files/detail/"+e.data().Id()))},loadHandler:function(e,t){function n(e){var n=b.findPage(e);n.isLoaded||n.isLoading||(b.markPageLoading(e),p(e).then(function(){b.markPageLoaded(e),t.pageLoaded(e)}))}if(console.log("loadHandler called. First visible: "+t.firstVisible.index+" (Page #"+t.firstVisible.page+"); Last visible: "+t.lastVisible.index+" (Page #"+t.lastVisible.page+")"),k()){var r=t.firstVisible.viewModel?b.findItemPage(t.firstVisible.viewModel):void 0,i=t.lastVisible.viewModel?b.findItemPage(t.lastVisible.viewModel):void 0;if(r&&i)for(var a=r.index;a<=i.index;a++)n(a+1)}}}});
...
define('modules/draft/viewmodels/draftPublicationDialog',["durandal/system","plugins/dialog","ko","modules/sitemap/viewmodels/siteMapTree","../datacontext","./draftPublicationViewModel","../contentGenerator","infrastructure/publicationService"],function(e,t,n,r,i,a,o,s){function l(e){var t=this;t.draft=n.observable(e),t.siteMapTree=new r,t.publications=n.observableArray(),t.hasOutdatedPublications=n.computed(function(){for(var e=t.publications(),n=0;n<e.length;n++)if(e[0].isOutdated())return!0;return!1})}return l.prototype.activate=function(){function t(t){return e.defer(function(e){i.fetchPublications(t.Id()).then(function(r){e.resolve(n.utils.arrayMap(r,function(e){return new a(t,e)}))}).fail(e.reject)}).promise()}var r=this,o=r.draft();r.siteMapTree.fetchSiteMapVersions(),t(o).then(function(e){r.publications(e)})},l.prototype.selectCreateNew=function(){var e=this;try{var n=o.createPublicationContent(e.draft());s.publish(n,e.siteMapTree.selectedNode()).then(function(){t.close(e,{dialogResult:!0})}).fail(function(e){alert(e.message)})}catch(r){alert(r.message)}},l.prototype.selectCancel=function(){t.close(this,{dialogResult:!1})},l.prototype.selectLink=function(){var e=this;try{var n=o.createPublicationContent(e.draft());s.setNodeContent(e.siteMapTree.selectedNode().Id(),n).then(function(){t.close(e,{dialogResult:!0})}).fail(function(e){alert(e.message)})}catch(r){alert(r.message)}},l});
...
define('modules/draft/viewmodels/editor/contentEditors/insertLinkDialog',["plugins/dialog","ko"],function(e,t){function n(){var n=this;n.url=t.observable("http://www.google.com").extend({required:!0}),n.ok=function(){e.close(n,{result:!0,url:n.url()})},n.cancel=function(){e.close(n,{result:!1})},t.validation.group(n)}return n});
...
define('modules/draft/viewmodels/editor/draftFiles',["durandal/app","../../module","ko","./editorModel","infrastructure/treeViewModel","infrastructure/serverUtil","infrastructure/keyboardHandler"],function(e,t,n,r,i,a,o){function s(s){function c(){var e=s.entity().distinctFileGroupNames();w(n.utils.arrayMap(e,g))}function u(){function e(e){return n.utils.arrayFirst(w(),function(t){return t.name().toLowerCase()===e.toLowerCase()})?!0:!1}var t=s.entity().distinctFileGroupNames(),r=n.utils.arrayFilter(t,function(t){return!e(t)}),i=v();n.utils.arrayForEach(r,function(e){w.push(g(e))});var a=y.tree();!i&&a&&(a.selectRootNode(),a.expandRootNodes())}function d(e){var t=n.utils.arrayFilter(y.groupNames(),function(t){var n=new RegExp("^("+e+")(\\s+\\(\\d+\\))?.*$","gi");return n.test(t)});return t.length>0}function f(){var e=new i.TreeViewModel,t=s.entity();e.keyName("Id"),e.createNode=function(){var t=new i.TreeNodeViewModel(e);return t.templateName=n.observable(),t},e.refresh=function(){var r;y.tree()&&(r=y.tree().saveState()),e.clear(),n.utils.arrayForEach(y.sortedFileGroups(),function(r){var i=p(e,r);e.root.addChildNode(i);var a=t.filesByGroupName(r.name());n.utils.arrayForEach(a,function(t){var n=h(e,t);n&&i.addChildNode(n)})}),r&&y.tree()&&y.tree().restoreState(r)},e.on("tree:nodeMoved",function(){b()}),y.tree(e)}function p(e,t){var r=e.createNode();return r.title=n.observable(t.name()),r.nodeType="group",r.entity(t),r.templateName=n.computed(function(){return r.title().length?"draftfilegroup-label":"draftfilegroup-label-empty"}),r.groupName=n.computed({read:function(){return r.entity().name()},write:function(n){n!==t.name()&&(r.entity().name(n),r.childNodes().forEach(function(e){e.entity().Group(n)}),e.refresh())}}),r}function h(e,t){if(!t)return null;var i=new r.DraftFileNode(t,t.getOrCreateResource("de"),e);return i.title=t.Name(),i.templateName=n.observable("draftfile-label"),i}function m(){var e=v();return e?e.title():""}function v(){if(!y.tree()||!y.tree().selectedNode())return null;var e=y.tree().selectedNode();return"group"===e.nodeType?e:e.parentNode()}function g(e){var t={name:n.observable(e)};return t.Id=n.computed(function(){return t.name()}),t}function b(){var e=y.tree(),t=0;e&&n.utils.arrayForEach(e.rootNodes(),function(e){n.utils.arrayForEach(e.childNodes(),function(e){var n=e.entity(),r=t++;n.Ranking()!==r&&n.Ranking(r)})})}var y=this,w=n.observableArray(),k=new o(t);y.name="DraftFiles",y.editor=s,y.server=a,y.determinations=n.observableArray(l),y.tree=n.observable(),y.deactivate=function(){k.deactivate()},s.currentContent.subscribe(function(e){e===y?k.activate():k.deactivate()});var x=!0;y.editor.entity().Files.subscribe(function(){x&&(u(),y.tree().refresh())}),y.groupNames=n.computed(function(){return n.utils.arrayMap(w(),function(e){return e.name()})}),y.sortedFileGroups=n.computed(function(){var e=w();return e.sort(function(e,t){var n=s.entity().rankingByGroupName(e.name()),r=s.entity().rankingByGroupName(t.name());return n===r?0:r>n?-1:1}),e}),y.selectFiles=function(){e.selectFiles({module:t,title:"Anhänge hinzufügen"}).then(function(e){if(e.dialogResult){var t=m(),r=s.entity().rankingByGroupName(t);n.utils.arrayForEach(e.selectedFiles,function(e){s.createDraftFile(e,t,r++)}),b()}})},y.addGroup=function(){function e(e){var t=n.utils.arrayFilter(y.groupNames(),function(t){var n=new RegExp("^("+e+")(\\s+\\(\\d+\\))?.*$","gi");return n.test(t)});return t.length?e+" ("+t.length+")":e}var t=e("Neue Gruppe");w.push(g(t)),y.tree().refresh();var r=y.tree().findNodeByKey(t);r&&(r.selectNode(),r.isExpanded(!0))},y.ensureGroupsExist=function(e){e.forEach(function(e){d(e)||w.push(g(e)),y.tree().refresh(),y.tree().selectRootNode()})},y.moveSelectedNodeUp=function(){var e=y.tree().selectedNode();e.moveUp&&e.moveUp()},y.moveSelectedNodeDown=function(){var e=y.tree().selectedNode();e.moveDown&&e.moveDown()},y.removeFile=function(t){var n="Entfernen",r="Abbrechen";e.showMessage("Soll der Anhang "+t.entity().Name()+" wirklich entfernt werden?","Anhang entfernen",[n,r]).then(function(e){e===n&&(t.detachFromParentNode(),t.entity().setDeleted())})},y.deleteGroup=function(){function t(){w.remove(r.entity()),r.detachFromParentNode(),x=!1,n.utils.arrayForEach(i,function(e){e.setDeleted()}),x=!0}var r=v();if(r){var i=n.utils.arrayMap(r.childNodes(),function(e){return e.entity()});if(i.length>0){var a="Gruppe löschen",o="Abbrechen";e.showMessage('Soll die Gruppe "'+r.title()+'" wirklich gelöscht werden?',"Gruppe löschen",[a,o]).then(function(e){e===a&&t()})}else t()}},y.contentTemplateName=function(){if(!y.tree()||!y.tree().selectedNode())return"";var e=y.tree().selectedNode();return"group"==e.nodeType?"draftfiles-group-template":"draftfiles-file-template"},y.navigateToFile=function(){if(y.tree()&&y.tree().selectedNode()&&"file"===y.tree().selectedNode().nodeType){var t=y.tree().selectedNode(),n=t.resource.FileVersion().File();e.trigger("caps:contentfile:navigateToFile",n)}},c(),f(),y.tree()&&(y.tree().refresh(),y.tree().expandRootNodes(),y.tree().selectRootNode()),k.keydown=function(e){var t=document.activeElement;y.tree()&&(!t||"INPUT"!=t.tagName&&"TEXTAREA"!=t.tagName)&&y.tree().handleKeyDown(e)}}var l=[{name:"Picture",title:"Bild"},{name:"Download",title:"Download"},{name:"Misc",title:"Sonstiges"}];return s});
...
define('modules/sitemap/viewmodels/index',["../module","ko","plugins/router","durandal/system","durandal/app","localization","./siteMapViewModel","./publicationViewModel","infrastructure/publicationService","authentication","../datacontext","durandal/composition","moment","infrastructure/keyboardHandler","infrastructure/scrollState","./addPageDialog"],function(e,t,n,r,i,a,o,s,l,u,c,d,f,p,h,m){function v(e){C(e)&&c.fetchSiteMapNode(e.Id()).then(function(){E()&&E().refreshTree(),y()})}function g(e){P(null),e&&e.fetchTree().then(function(){if(P()){var t=e.tree().findNodeByKey(P().Id());if(t)return t.selectNode(),void 0}e.selectRootNode()})}function b(e){R(null),y(),w(e)}function y(){_(null),P()&&P().ContentId()&&c.fetchPublication(P().ContentId()).then(function(){var e=new s(P());_(e)}).fail(N)}function w(e){L(null),e&&L(new M(e))}function k(e){var n=t.utils.arrayFirst(U.siteMapVersions(),function(t){return t.entity()===e});E(n)}function x(e){var t=new o(e);return t.selectedNodeChanged=function(e){e&&P(e.entity())},t}function C(e){return E()&&E().containsNode(e)}function N(e){r.log(e.message),alert(e.message)}function D(e){var t=E();if(t&&e){var n=e.nextNode()||e.previousNode()||e.ParentNode();c.deleteSiteMapNode(e).then(function(){t.refreshTree(),n&&t.selectNodeByKey(n.Id()),i.trigger("caps:sitemapnode:deleted",e)}).fail(N)}}function I(){c.fetchFirstWebsite().then(function(e){T(e.results[0]),k(T().latestSiteMap())}).fail(N)}function F(e){c.fetchSiteMapNodeByContentId(e).then(function(e){if(!e.results.length)return I(),void 0;var t=e.results[0],n=t.SiteMap();T(n.Website()),k(n),P(t)}).fail(N)}function S(e){var n=this;n.entity=e,n.isSelected=t.computed(function(){return R()===n}),n.selectTeaser=function(){R(n),w(n.entity)}}function M(n){var r=this;r.entity=n,r.supportedTranslations=a.website.supportedTranslations(),r.edit=function(){n&&e.router.navigate("#sitemap/edit/"+n.Id())},r.editTranslation=function(t){n&&e.router.navigate("#sitemap/translate/"+n.Id()+"/"+t.culture)},r.createTeaser=function(){if(n){var e=n.SiteMap().Id(),t=n.SiteMap().rootNodes()[0].Id();l.createTeaser(e,t,n.Id())}},r.canCreateTeaser=function(){return n?"ROOT"==n.NodeType()||n.hasTeasers()?!1:n.isTeaser()?!1:!0:!1},r.deleteTeaser=function(){var e="Aufmacher löschen",t="Abbrechen";i.showMessage('Soll der Aufmacher "'+r.entity.localeTitle("de")+'" wirklich gelöscht werden?',"Aufmacher löschen",[e,t]).then(function(t){t===e&&D(r.entity)})},r.hasOptions=t.computed(function(){return r.entity.isTeaser()}),r.hasContent=t.computed(function(){return n&&n.Content()}),r.contentSummary=t.computed(function(){if(!n.ContentId())return"Kein Inhalt festgelegt";var e=n.Content();return e?e.EntityType()+" #"+e.EntityKey()+" (v."+e.ContentVersion()+")":""}),r.authorSummary=t.computed(function(){if(!n.ContentId()||!n.Content())return"";var e=n.Content();return e.AuthorName()+" "+f(e.ContentDate()).fromNow()}),r.selectContent=function(){function t(e){l.setNodeContent(r.entity.Id(),e).fail(function(e){alert(e.message)})}isActive=!1,i.selectContent({module:e}).then(function(e){e.dialogResult&&t(e.selectedContent)}).finally(function(){isActive=!0})},r.editContent=function(){n.ContentId()&&n.Content()&&i.editContent(n.Content().EntityType(),n.Content().EntityKey())},r.deletePublication=function(){function e(){n.ContentId()&&n.Content()&&c.deletePublication(n)}var t="Entfernen",r="Abbrechen";i.showMessage("Soll der Inhalt dieser Seite wirklich entfernt werden?","Inhalt entfernen?",[t,r]).then(function(n){n===t&&e()})}}var T=t.observable(),E=t.observable(),P=t.observable(),_=t.observable(),R=t.observable(),L=t.observable(),A=!1,V=new p(e),O=new h(e);E.subscribe(g),P.subscribe(b),i.on("caps:sitemapnode:saved",function(e){C(e)&&c.fetchSiteMapNode(e.Id())}),i.on("caps:publication:created",v),i.on("caps:publication:refreshed",v);var B=$(window);e.on("module:compositionComplete",function(e,t){t===U&&(B.trigger("forceViewportHeight:refresh"),O.activate())});var U={website:T,siteMapVersions:t.computed(function(){var e=T()?T().sortedSiteMapVersions():[];return t.utils.arrayMap(e,x)}),selectedSiteMap:E,selectedNode:P,selectedPublication:_,properties:L,listScrollState:O,teasers:t.computed(function(){if(!P())return[];var e=t.utils.arrayFilter(P().childNodes(),function(e){return e.isTeaser()});return t.utils.arrayMap(e,function(e){return new S(e)})}),activate:function(e){A||(A=!0,e&&e.p||I()),e&&e.p&&F(e.p),V.activate()},shouldActivate:function(){return!0},deactivate:function(){V.deactivate(),O.deactivate()},createSiteMapVersion:function(){c.createNewSiteMapVersion(T()).then(k).fail(N)},deleteSiteMapVersion:function(){var e="Sitemap-Version verwerfen",t="Abbrechen",n=E().entity();n&&i.showMessage("Soll diese Version wirklich verworfen werden?","Sitemap-Version verwerfen?",[e,t]).then(function(t){if(t===e){var r=n.nextVersion()||n.previousVersion();k(r),c.deleteSiteMapVersion(n).fail(N)}})},publishSiteMapVersion:function(){c.publishSiteMap(E().entity(),u.user().userName())},createSiteMapNode:function(){function e(e){c.createSiteMapNode(E().entity(),P(),e,e).then(function(e){E().refreshTree(),E().selectNodeByKey(e.Id())}).fail(N)}E()&&P()&&m.show().then(function(t){t&&e(t.title)})},deleteSiteMapNode:function(){var e="Seite löschen",t="Abbrechen";i.showMessage('Soll die Seite "'+P().localeTitle("de")+'" wirklich gelöscht werden?',"Seite löschen",[e,t]).then(function(t){t===e&&D(P())})},editWebsite:function(){n.navigate("#website")},moveSelectedNodeUp:function(){P()&&(P().moveUp(),c.saveChanges().then(function(){E()&&E().refreshTree()}))},moveSelectedNodeDown:function(){P()&&(P().moveDown(),c.saveChanges().then(function(){E()&&E().refreshTree()}))},moveSelectedNode:function(){function t(e){return!e.isTeaser()&&e.Id()!==P().Id()}if(E()){var n=E(),r=P();i.selectSiteMapNode({module:e,nodeFilter:t,siteMapId:n.entity().Id(),canSelectSiteMap:!1}).then(function(e){e.dialogResult&&e.selectedNode&&e.selectedNode.Id()!==r.ParentNodeId()&&(r.reparent(e.selectedNode),c.saveChanges().then(function(){n.refreshTree(),n.tree().selectedNode().ensureVisible()}))})}}};return V.keydown=function(e){var t=document.activeElement;(!t||"INPUT"!=t.tagName&&"TEXTAREA"!=t.tagName)&&E()&&E().tree()&&E().tree().handleKeyDown(e)},U});
